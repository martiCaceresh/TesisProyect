...
namespace OptiArroz_Tesis_Proyect.Controllers
{
    public class WarehouseManagerController : Controller
    {
        ...
        public WarehouseManagerController(IFirstConfigurationDA FirstConfigurationDA, IZoneDA ZoneDA, UserManager<ApplicationUser> UserManager)
        {
        ...
        public async Task<IActionResult> Create(string Name, string Description, decimal Length, decimal Width, decimal Height, int Rows, int Columns)
        {
            try
            {
                var CurrentUser = await UserManager.GetUserAsync(User);
                await ZoneDA.Create(new Zone(Name, Description, Length, Width, Height, Rows, Columns, CurrentUser.Id));
